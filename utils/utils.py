...
from models.driving.LTFed import Driving_LTFed
...
def args_to_string(args):
    ...
    args_to_show = ["experiment", "network_name", "fit_by_epoch", "bz_train",
                    "lr", "decay", "local_steps", "temporal", "fl", "num_previous_frames"]
...
def get_model(name, model, device, epoch_size, optimizer_name="adam", lr_scheduler="custom",
    ...
    if "driving" in name and model == "LTFed":
        criterion = [nn.MSELoss(), nn.KLDivLoss(reduction='batchmean')]
        metric = [mean_squared_error, mean_absolute_error]
        return Driving_LTFed(criterion, metric, device, optimizer_name, lr_scheduler, initial_lr, epoch_size)
    else:
        raise NotImplementedError


def get_iterator(name, path, device, batch_size, temporal=False, num_previous_frames=10):
    if "driving_gazebo" in name or "driving_carla" in name or "driving_udacity" in name:
        return get_iterator_complex_driving(path, device, batch_size=batch_size, temporal=temporal, num_previous_frames=num_previous_frames)
    else:
        raise NotImplementedError
